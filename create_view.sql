CREATE VIEW `streaming-project-451710.dataset.customer_courier_conversations_view` AS
SELECT
...
FROM
  `streaming-project-451710.dataset.orders` AS o
JOIN
  `streaming-project-451710.dataset.conversations` AS m
ON
  o.orderId = m.orderId
LEFT JOIN (
  SELECT
  ...
  FROM (
    ...
    FROM
      `streaming-project-451710.conversations`
  ) WHERE rn = 1
) AS last_stage_table
ON
